...
for file in files:
    with open(file, 'r', encoding='utf-8') as f:
        content = f.read()
        blocks = re.findall(r'```html\s*(.*?)\s*```', content, re.DOTALL)
...
# Inject into rows 80 to 119 (which are index 80 to 119 in the rows list)
...
print("programatic.csv updated with batch 3 (rows 80-119).")
